        ...
        if ($this->managerTheme->getCore()->hasPermission('exec_module')) {
            ...
            if (count($modules) > 0) {
                foreach ($modules as $row) {
                    $items['module' . $row['id']] = [
                        ...
                        manager_route('execute_modules', ['id' => $row['id']]),
                        e($row['name']),
                        '',
                        '',
                        'main',
            ...
            foreach ($this->managerTheme->getCore()->modulesFromFile as $module) {
                ...
                $items['module' . $module['id']] = [
                    ...
                    !empty($module['properties']['routes']) ? 'modules/' . $module['id'] : manager_route('execute_modules', ['id' => $module['id']]),
                    $module['name'],
                    '',
                    '',
                    'main',
        ...
        $this->sitemenu['refresh_site'] = [
            ...
            [
                'refresh_site_in_window' => [
                    ...
                    "modx.popup({url:'" . manager_route('refresh_site') . "', title:'" . $this->managerTheme->getLexicon('refresh_site') . "', icon: 'fa-recycle', iframe: 'ajax', selector: '.tab-page>.container', position: 'right top', width: 'auto', maxheight: '50%%', wrap: 'body' })",
                    // onclick
                    $this->managerTheme->getLexicon('refresh_site'),
                    // title
