...
if ($_SESSION['mgrRole'] != 1 && is_numeric($id)) {
    ...
    if (empty($moduleAccess)) {
        $modx->webAlertAndQuit("You do not sufficient privileges to execute this module.", manager_route('elements', [ 'tab' => 5 ]));
    }
}
if (is_numeric($id)) {
    // get module data
    ...
    if (is_null($content)) {
        $modx->webAlertAndQuit("No record found for id {$id}.", manager_route('elements', [ 'tab' => 5 ]));
    }
    $content = $content->toArray();
    if ($content['disabled']) {
        $modx->webAlertAndQuit("This module is disabled and cannot be executed.", manager_route('elements', [ 'tab' => 5 ]));
    }
} else {
