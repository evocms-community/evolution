...
$plugins = SitePlugin::query()
    ->select('site_plugins.id')
    ->leftJoin('site_plugins as t2', function (JoinClause $join) {
        $join
            ->on('site_plugins.name', '=', 't2.name')
            ->on('site_plugins.id', '<', 't2.id');
    })
    ->whereNull('t2.id')
    ->get();
...
foreach ($plugins as $row) {
    $latestIds[] = $row->getKey();
}

// Get list of plugins with disabled and enabled versions
$plugins = SitePlugin::query()
    ->select('site_plugins.id')
    ->join('site_plugins as t2', function (JoinClause $join) {
        $join
            ->on('site_plugins.name', '=', 't2.name')
            ->on('site_plugins.id', '!=', 't2.id');
    })
