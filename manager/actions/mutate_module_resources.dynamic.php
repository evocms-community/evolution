...
switch ($_REQUEST['op'] ?? '') {
    ...
    case 'del':
        ...
        $ds = SiteModule::query()->whereIn('id', $opids)->get();
        // loop through resources and look for plugins and snippets
        $plids = [];
        $snids = [];
        foreach ($ds->toArray() as $i => $row) {
            if ($row['type'] == '30') {
        ...
        // get guid
        $guid = SiteModule::query()->find($id)->guid;
        ...
        SiteModuleDepobj::query()->whereIn('id', $opids)->delete();
        break;
}
...
<script>
  ...
  function openSelector (resource, mode, callback, w, h) {
    ...
    win = window.open(url, 'resource_selector', 'left=' + x + ',top=' + y + ',height=' + h + ',width=' + w +
      ',status=yes,scrollbars=yes,resizable=yes,toolbar=no,menubar=no,location=no')
    win.opener = self
  }
