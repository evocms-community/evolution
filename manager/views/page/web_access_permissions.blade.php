...
@section('content')
    ...
    @endpush
    <h1>
        <i class="{{ $_style['icon_web_user_access'] }}"></i> {{ ManagerTheme::getLexicon('web_access_permissions') }} <i
    ...
    <div class="tab-pane" id="wuapPane">
        ...
        <div class="tab-page" id="tabPage1">
            ...
            <div class="container container-body">
                <p class="element-edit-message-tab alert alert-warning">
                    {{ ManagerTheme::getLexicon('access_permissions_users_tab') }}</p>
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group-btn">
                                <input class="btn btn-success" type="submit"
                                    value="{{ ManagerTheme::getLexicon('submit') }}" />
                            </div>
                        </div>
                    </form>
                </div>
                    ...
                    @foreach ($userGroups as $userGroup)
                        <div class="form-group">
                            <form method="post" action="index.php" name="accesspermissions">
                                ...
                                <div class="input-group">
                                    <input class="form-control" type="text" name="newgroupname"
                                        value="{{ $userGroup->name }}" />
                                    <div class="input-group-btn">
                                        <input class="btn btn-secondary" type="submit"
                                            value="{{ ManagerTheme::getLexicon('rename') }}" />
                                        <input class="btn btn-danger" type="button"
                                            value="{{ ManagerTheme::getLexicon('delete') }}"
                                            onclick="deletegroup({{ $userGroup->getKey() }}, 'usergroup');" />
                                    </div>
                                </div>
                            </form>
                            ...
                            @else
                                ...
                                @foreach ($userGroup->users as $user)
                                    <a href="index.php?a=88&id={{ $user->getKey() }}">{{ $user->username }}</a>
                                    @if ($loop->last === false)
                                        ,
                                    @endif
                                @endforeach
        ...
        <div class="tab-page" id="tabPage2">
            ...
            <div class="container container-body">
                <p class="element-edit-message-tab alert alert-warning">
                    {{ ManagerTheme::getLexicon('access_permissions_resources_tab') }}</p>
                <div class="form-group">
                    ...
                    @foreach ($documentGroups as $documentGroup)
                        <div class="form-group">
                            <form method="post" action="index.php" name="accesspermissions">
                                ...
                                <div class="input-group">
                                    <input class="form-control" type="text" name="newgroupname"
                                        value="{{ $documentGroup->name }}" />
                                    <div class="input-group-btn">
                                        <input class="btn btn-secondary" type="submit"
                                            value="{{ ManagerTheme::getLexicon('rename') }}" />
                                        <input class="btn btn-danger" type="button"
                                            value="{{ ManagerTheme::getLexicon('delete') }}"
                                            onclick="deletegroup({{ $documentGroup->getKey() }},'documentgroup');" />
                                    </div>
                                </div>
                            </form>
                            ...
                            @else
                                ...
                                @foreach ($documentGroup->documents as $document)
                                    <a href="index.php?a=3&id={{ $document->getKey() }}"
                                        title="{{ $document->pagetitle }}">{{ $document->getKey() }}</a>
                                    @if ($loop->last === false)
                                        ,
                                    @endif
                                @endforeach
        ...
        @if ($documentGroups->count() > 0 && $userGroups->count() > 0)
            <div class="tab-page" id="tabPage3">
                ...
                <div class="container container-body">
                    <p class="element-edit-message-tab alert alert-warning">
                        {{ ManagerTheme::getLexicon('access_permissions_links_tab') }}</p>
                    <div class="form-group">
                        ...
                        <form method="post" action="index.php" name="accesspermissions">
                            ...
                            <label for="context_mgr"><input id="context_mgr" type="checkbox" value="0"
                                    name="context[]" /> mgr </label>
                            <label for="context_web"><input id="context_web" type="checkbox" value="1"
                                    name="context[]" /> web </label>
                                ...
                                @if ($userGroup->documentGroups->count() > 0)
                                    <ul>
                                        ...
                                        @foreach ($userGroup->documentGroups as $documentGroup)
                                            <li>
                                                {{ $documentGroup->name }}
                                                ({{ $documentGroup->pivot->context ? 'web' : 'mgr' }})
                                                <small><i>(<a class="text-danger"
                                                            href="index.php?a=92&coupling={{ $documentGroup->pivot->id }}&context={{ $documentGroup->pivot->context }}&operation=remove_document_group_from_user_group">{{ ManagerTheme::getLexicon('remove') }}</a>)</i></small>
                                            </li>
                                        @endforeach
                                    </ul>
                                @else
