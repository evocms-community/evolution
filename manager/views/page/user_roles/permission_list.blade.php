...
<div class="tab-page {{ $tabPageName }}" id="{{ $tabIndexPageName }}">
    <h2 class="tab">
        <a href="?a=86&tab={{ $index }}">
            <i class="fa fa-user-tag"></i>{{ ManagerTheme::getLexicon('manage_permission') }}
        </a>
    </h2>
        ...
        <div id="{{ $tabIndexPageName }}_content" class="resourceTable panel panel-default">
            ...
            @if(isset($groups))
                @foreach($groups as $group)
                    @component('manager::partials.panelCollapse', ['name' => $tabIndexPageName . '_content', 'id' => $group->id, 'title' => ManagerTheme::getLexicon($group->lang_key) ])
                        <ul class="elements">
