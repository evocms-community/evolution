                        ...
                        <tr>
                            <td class="tableItem text-center" style="width: 34px;">
                                @if (evo()->hasAnyPermissions(['edit_module', 'exec_module']))
                                    <a class="tableRowIcon" href="javascript:;"
                                        onclick="return showContentMenu({{ $module->getKey() }}, event);"
                                        title="{{ __('global.click_to_context') }}">
                            ...
                            <td class="tableItem">
                                @if (evo()->hasAnyPermissions(['edit_module']))
                                    <a href="index.php?a=108&id={{ $module->getKey() }}"
                                        title="{{ __('global.module_edit_click_title') }}">{{ $module->name }}</a>
                                @else
                                    {{ $module->name }}
                                @endif
                            </td>
    ...
    <script>
        ...
        function showContentMenu(id, e) {
            ...
            contextm.style.left = (e.pageX || (e.clientX + (document.documentElement.scrollLeft || document.body
                .scrollLeft))) {{ ManagerTheme::getTextDir('+10') }} + 'px'; //offset menu if RTL is selected
