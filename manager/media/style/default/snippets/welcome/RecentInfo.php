...
$tpl = '<tr>
    ...
    <td data-toggle="collapse" data-target=".collapse[+id+]"><a class="[+status+]" title="[%edit_resource%]" href="'. manager_route('about_document') .'&id=[+id+]" target="main">[+pagetitle:htmlentities+]</a></td>
...
$btntpl['edit'] = '<a title="[%edit_resource%]" href="'. manager_route('edit_document') .'&id=[+id+]" target="main"><i class="'. $_style['icon_edit'] . $_style['icon_size_fix'] . '"></i></a> ';
...
foreach ($contents->get()->toArray() as $ph) {
    ...
    if ($modx->hasPermission('delete_document')) {
        if ($ph['deleted'] == 0) {
            $delete_btn = '<a onclick="return confirm(\'[%confirm_delete_record%]\')" title="[%delete_resource%]" href="'. manager_route('delete_resource') .'&id=[+id+]" target="main"><i class="'. $_style['icon_trash'] . $_style['icon_size_fix'] . '"></i></a> ';
        } else {
            $delete_btn = '<a onclick="return confirm(\'[%confirm_undelete%]\')" title="[%undelete_resource%]" href="'. manager_route('undelete_resource') .'&id=[+id+]" target="main"><i class="'. $_style['icon_undo'] . $_style['icon_size_fix'] . '"></i></a> ';
        }
