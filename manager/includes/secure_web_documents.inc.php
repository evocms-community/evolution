...
function secureWebDocument($docid = '', $context = 1)
{
    ...
    if (is_numeric($docid) && $docid > 0) {
        SiteContent::withTrashed()->find($docid)->update([$privateField => 0]);
    } else {
        SiteContent::withTrashed()->where($privateField, 1)->update([$privateField => 0]);
    }
    ...
    if (count($ids) > 0) {
        SiteContent::withTrashed()->whereIn('id', $ids)->update([$privateField => 1]);
    }
}
